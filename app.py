...
if not os.path.exists(csv_path):
    st.error("❌ File `mall_customers.csv` not found in the directory.")
    st.stop()
...
if selected_features:
    ...
    if st.button("Predict Cluster"):
        ...
        # Optional: Show input location on the plot
        if len(selected_features) >= 2:
            plt.figure(figsize=(10, 6))
            sns.scatterplot(data=df, x=x_axis, y=y_axis, hue='Cluster', palette='tab10')
            plt.scatter(
    ...
    st.warning("Please select at least one numerical feature to proceed.")
